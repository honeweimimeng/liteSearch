...
func (i *IndexReader) GetDoc(address index.DocAddress) *schema.Document {
	reader := i.searcher.segmentReaders[address.SegmentOrd]
	return reader.GetDoc(address.DocID)
}

func (i *IndexReader) DocTotal() uint32 {
	var total uint32
	for i2 := range i.searcher.SegmentReaders() {
		total = total + i.searcher.SegmentReaders()[i2].DocNum()
	}
	return total
}
